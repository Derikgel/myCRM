...
**myCRM** — русскоязычная CRM-система на базе [Krayin CRM](https://github.com/krayin/laravel-crm)
(Laravel + Vue.js, лицензия MIT). Интерфейс переведён на русский язык
(`lang/ru`, `packages/Webkul/{Admin,Installer,WebForm,GoogleContact}/src/Resources/lang/ru`).

Оригинальный проект — Krayin CRM (© Webkul, MIT License, см. `LICENSE`).
